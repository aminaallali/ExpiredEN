  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
      {STATS.map((stat) => {
        ...
        const count = data ? data[stat.phase].toLocaleString() : '—'

        if (isActive) {
          return (
            <div
              key={stat.phase}
              className="bg-brand-yellow rounded-4xl p-6 relative text-brand-dark flex flex-col justify-between h-64"
            >
        ...
        return (
          <div
            key={stat.phase}
            className="bg-brand-surface rounded-4xl p-6 relative text-brand-text flex flex-col justify-between h-64 border border-brand-surface-light"
          >
            <div>
              <div className="text-sm font-medium text-brand-muted mb-1">{stat.label}</div>
              <div className="text-4xl font-light">
                {count} <span className="text-lg text-brand-muted">names</span>
              </div>
            </div>
